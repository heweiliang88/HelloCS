<div align="center" ><font face="微软雅黑" size="7">
  Hello World
</font></div>

<img alt="今日诗词" src="https://v2.jinrishici.com/one.svg?font-size=20&spacing=2&color=black" style="max-width:100%; display: block; margin: 0 auto;">



<p align="center">
<a title="Hits" target="_blank" href="https://github.com/Achuan-2/Achuan-2"><img src="https://hits.b3log.org/Achuan-2/Achuan-2.svg" ></a>
<a title="github" target="_blank" href="https://github.com/Achuan-2"><img src="https://img.shields.io/badge/dynamic/json?label=GitHub&suffix=%20followers&query=%24.data.totalSubs&url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dgithub%26queryKey%3DAchuan-2&labelColor=282c34&color=353940&logo=github&longCache=true" ></a>
<a title="weibo" target="_blank" href="https://weibo.com/2139813304/profile"><img src="https://img.shields.io/badge/dynamic/json?labelColor=e71f19&color=353940&label=Weibo&suffix=%20followers&query=%24.data.totalSubs&url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dweibo%26queryKey%3D2139813304&logo=sina-weibo&longCache=true" ></a>
<a title="zhihu" target="_blank" href="https://www.zhihu.com/people/achuan-2"><img src="https://img.shields.io/badge/dynamic/json?color=353940&labelColor=0084ff&label=Zhihu&suffix=%20followers&query=%24.data.totalSubs&url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dzhihu%26queryKey%3Dachuan-2&logo=data:image/svg+xml;base64,&longCache=true" ></a>
<a title="bilibili" target="_blank" href="https://space.bilibili.com/349243695"><img src="https://img.shields.io/badge/dynamic/json?color=353940&labelColor=f27596&label=Bilibili&suffix=%20followers&query=%24.data.totalSubs&url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dbilibili%26queryKey%3D349243695&logo=data:image/png;base64,&longCache=true" ></a>
<a title="coolapk" target="_blank" href="https://www.coolapk.com/u/787649"><img src="https://img.shields.io/badge/dynamic/json?color=30343b&labelColor=17a15e&label=Coolapk&suffix=%20followers&query=%24.data.totalSubs&url=https%3A%2F%2Fapi.spencerwoo.com%2Fsubstats%2F%3Fsource%3Dcoolapk%26queryKey%3D787649&logo=data:image/svg+xml;base64,&longCache=true" ></a>
</p>


<div>
<div style="float:right">
<img height="170px" src="https://github-readme-stats.vercel.app/api?username=heweiliang88" />
  </div>
  <span>
<img height="170px" src="https://github-readme-stats.vercel.app/api/top-langs/?username=heweiliang88&layout=compact&langs_count=8" />
    </span>
</div>

<div align="center">
    <img  src="https://github-readme-streak-stats.herokuapp.com/?user=heweiliang88" />
</div>

<div align="center">
    <img src="https://activity-graph.herokuapp.com/graph?username=heweiliang88&theme=minimal" />
</div>
## 通识

学习点

- 网上教程（3篇~5篇） ==> 书籍 （一本到两本）
- 学习视频 ==> 实战视频  ==> 个人实战项目
- 把计算机所有的笔记看一遍，然后存档
- 学习番茄时 25min 5min休息

工作流

- 

学习流

- 学习 =》项目 =》复习 =》复述

MacOS

```bash
PS/2 键盘
⇧ (shift)
⌃ (control == ctrl )  ⌥ (Option == win )  ⌘ (Command / Cmd = alt)
tab
shift
control ==> ctrl 控制
option ==>  win 选项
command ==> alt 命令 
```

## 资源

书籍

- [ ] [free-programming-books](https://github.com/EbookFoundation/free-programming-books)
- [ ] [justjavac/free-programming-books-zh_CN: 免费的计算机编程类中文书籍，欢迎投稿](https://github.com/justjavac/free-programming-books-zh_CN)
- [ ] [itdevbooks/pdf: 编程电子书](https://github.com/itdevbooks/pdf)
- [ ] [sindresorhus/awesome: 😎 Awesome lists about all kinds of interesting topics](https://github.com/sindresorhus/awesome)
- [ ] [Jackpopc/CS-Books-Store: 你想要的计算机经典书籍，这里都有！](https://github.com/Jackpopc/CS-Books-Store)

- [命名函数表达式探秘](http://justjavac.com/named-function-expressions-demystified.html) - kangax、为之漫笔(翻译) (原始地址无法打开，所以此处地址为justjavac博客上的备份)
- [你不知道的Javascript](https://github.com/getify/You-Dont-Know-JS/tree/1ed-zh-CN)
- [深入理解JavaScript系列](http://www.cnblogs.com/TomXu/archive/2011/12/15/2288411.html)
- [现代 JavaScript 教程](https://zh.javascript.info/) - Ilya Kantor
- [学用 JavaScript 设计模式](http://www.oschina.net/translate/learning-javascript-design-patterns) - 开源中国
- [Airbnb JavaScript 规范](https://github.com/adamlu/javascript-style-guide)
- [ECMAScript 6 入门](http://es6.ruanyifeng.com/) - 阮一峰
- [Google JavaScript 代码风格指南](http://bq69.com/blog/articles/script/868/google-javascript-style-guide.html)
- [JavaScript 标准参考教程（alpha）](http://javascript.ruanyifeng.com/)
- [javascript 的 12 个怪癖](https://github.com/justjavac/12-javascript-quirks)
- [JavaScript 秘密花园](http://bonsaiden.github.io/JavaScript-Garden/zh/)
- [《JavaScript 模式》](https://github.com/jayli/javascript-patterns) (《JavaScript patterns》译本)
- [Javascript 原理](https://web.archive.org/web/20170112164945/http://typeof.net/s/jsmech/)
- [JavaScript Promise迷你书](http://liubin.github.io/promises-book/)
- [Javascript编程指南](http://pij.robinqu.me/) ([源码](https://github.com/RobinQu/Programing-In-Javascript))
- [JavaScript核心概念及实践](http://icodeit.org/jsccp/) (PDF)

## 工具

### 命令行

- wget
- curl
- 

### Mac OS

- Google 
- ITERM2

  - [舒适美观的mac终端, iTerm2+zsh+powerlevel9k+vim+virtualenv - 掘金](https://juejin.cn/post/6844903941528879112)
  - [eillsu/iTerm2-Chinese-Tutorial: iTerm2 中文教程](https://github.com/eillsu/iTerm2-Chinese-Tutorial)

- Draw.io
- Postman
- Dash
- Vscode
- Typora
- Sourcetree
- GitMind

### Window10



### Linux 











## 笔记

### Markdown

README

- [ ] [如何写一个通用的README规范 - 掘金](https://juejin.cn/post/6844903576167251976)
- [ ] [如何写一个高逼格 README - 掘金](https://juejin.cn/post/6844903585000456199)
- [ ] [前方高能！一个前端程序员的 README 能写到多好看 - 掘金](https://juejin.cn/post/7022299474458312718)
- [ ] [【译】10 款国外实用、有趣的 GitHub 简介 README - 掘金](https://juejin.cn/post/6857655197020356621)
- [ ] [yokiyokiyoki/readme-template: useful template for Readme](https://github.com/yokiyokiyoki/readme-template)

### FLOW

- mermaid
- flow
- sequence

计算机必备绘制图

- 流程图
- 

### LateX



### 图片制作





### 字体编辑



### 文字编辑





### 视频制作









- vim vs macvim
  - vim 配置文件 

```bash
vundler
vim
macvim
source ~/.bash_profile
```

- vundler  
- vim-plug [junegunn/vim-plug: Minimalist Vim Plugin Manager](https://github.com/junegunn/vim-plug)

## 408

数据结构与算法 => 软件工程 => 计算机网络 => 计算机组成原理 => 操作系统

### 软件工程

- [ ] [软件工程导论 张海藩_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Ns41177VM?spm_id_from=333.337.search-card.all.click)

### 数据结构与算法

- 

### 计算机网络

- [ ] [王道计算机考研 计算机网络_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV19E411D78Q?spm_id_from=333.999.0.0)

### 计算机组成原理

- [ ] [王道计算机考研 计算机组成原理_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1BE411D7ii?spm_id_from=333.999.0.0)

### 操作系统

- 

## 进度

项目管理工具

- 甘特图
- WBS任务分解
- 时间线
- 状态图
- 日历
- PERT图

## 原型

- [ ] UI设计 sketch
- [ ] [【Sketch教程】全网上万好评优质Sketch教程[全50集]_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1ov411q7kX?spm_id_from=333.337.search-card.all.click)
- [ ] [UI设计教程 零基础学习sketch系统教程_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV17W411v7RP?spm_id_from=333.337.search-card.all.click)

## 前端

学习路径

git => html5 => css3 => javascript => jquery => es6 => typescript => vue => webpack => node 

### 前端书籍

------

- css世界
- css揭秘
- css权威指南
- web 性能权威指南
- JavaScript语言精粹
- **现代javascript教程（在线）**

https://zh.javascript.info/

- **JavaScript忍者秘籍（第2版）**
- Web开发技术丛书 HTML 5与CSS 3权威指南 上册
- 图解css3
- JavaScript高级程序设计
- [ ] 你所不知道的JS 上种
- javascirpt设计思想
- 最简单粗暴的路线，就是红黄绿三步走，红宝书体系化，小黄书进阶语言细节，绿宝书学习，其他的作为横向扩展
- 锋利的jquery
- [你不知道的JavaScript 上卷+中卷+下卷（套装共3册）](https://github.com/getify/You-Dont-Know-JS)
- JavaScript DOM编程艺术 第2版
- 周爱民老师的《Javascript核心原理解析》
- 李兵的《浏览器工作原理与实践》
- winter的《重学前端》
- ‌es6 标准入门
- 深入理解es6
- 深入浅出webpack
- vue 权威指南
- vue3.0 从入门到实战
- 剑指office
- 前端程序员面试题库
- typescript入门到实战
- 深入浅出 node.js
- html权威指南

Time

- 图解css 1天 css世界 css揭秘 css权威指南
- javascript 高级程序设计 3天 红黄绿宝书
- es6 标准入门 2天
- 深入理解 ES6 2天
- vue 权威指南 看官网 2天
- vue 3.0从入门到实战 2天
- typescript入门与实战 2天
- 深入浅出webpack 2天 https://webpack.wuhaolin.cn
- node.js从入门到精通 2天
- 深入浅出node.js 2天
- 微信小程序指南 微信公众号 小程序开发
- JavaScript设计模式 2天
- 计算机网络 HTTP权威指南 图解 http 图解 tcp/ip 4天
- 算法 算法图解 javascript算法指南 1天漫画算法 labuladong的算法小抄
- 剑指office 3天
- 前端程序员面试题库 3天
- Taro多端开发权威指南：小程序、H5与App高效开发实战
- 微信小程序项目开发实战——用WePY、mpvue、Taro打造高效的小程序
- Flutter企业级应用开发实战——闲鱼技术发展与创新
- 深入React技术栈
- React Native实战：JavaScript开发iOS和Android应用
- https://jkchao.github.io/typescript-book-chinese/#why
- vue.js实战
- 前端程序员面试秘籍
- 前端程序员面试笔试真题库
- 剑指office

### github

- [x] [前言 | 千古前端图文教程](https://web.qianguyihao.com/#%E5%89%8D%E8%A8%80)
- [ ] [labuladong/fucking-algorithm: 刷算法全靠套路，认准 labuladong 就够了！English version supported! Crack LeetCode, not only how, but also why.](https://github.com/labuladong/fucking-algorithm)
- [ ] [justjavac/free-programming-books-zh_CN: 免费的计算机编程类中文书籍，欢迎投稿](https://github.com/justjavac/free-programming-books-zh_CN#javascript)
- [ ] [CyC2018/CS-Notes: 技术面试必备基础知识、Leetcode、计算机操作系统、计算机网络、系统设计](https://github.com/CyC2018/CS-Notes)

- [x] [phodal/github: GitHub 漫游指南- a Chinese ebook on how to build a good project on Github. Explore the users' behavior. Find some thing interest.](https://github.com/phodal/github)
- [x] [phodal/fe: 《我的职业是前端工程师》 - Ebook：I'm a FrontEnd Developer](https://github.com/phodal/fe)
- [ ] [freeCodeCamp/freeCodeCamp: freeCodeCamp.org's open-source codebase and curriculum. Learn to code for free.](https://github.com/freeCodeCamp/freeCodeCamp)
- [ ] [EbookFoundation/free-programming-books: Freely available programming books](https://github.com/EbookFoundation/free-programming-books)
- [ ] [jwasham/coding-interview-university: A complete computer science study plan to become a software engineer.](https://github.com/jwasham/coding-interview-university)
  [sindresorhus/awesome: 😎 Awesome lists about all kinds of 
- [ ] [interesting topics](https://github.com/sindresorhus/awesome)
  [getify/You-Dont-Know-JS: A book series on JavaScript. @YDKJS on twitter.](https://github.com/getify/You-Dont-Know-JS)



### git



### html5

html



html5

### css3

css3



less/scss



### javaScript

- [ ] [ruanyf/es6tutorial: 《ECMAScript 6入门》是一本开源的 JavaScript 语言教程，全面介绍 ECMAScript 6 新增的语法特性。](https://github.com/ruanyf/es6tutorial)

### jquery



### vue



### webpack



### react



### node 



### typescript



## 后端

Shell => Python => Java => C => C++ => JavaScript => Go => Android => Kotlin 

- 了解各个编程语言的应用范围
- 各编程语言的流程图

### C





### C++





### JavaScript

- [ ] [JavaScript教程 - 廖雪峰的官方网站](https://www.liaoxuefeng.com/wiki/1022910821149312)



### Java

- [ ] [Java教程 - 廖雪峰的官方网站](https://www.liaoxuefeng.com/wiki/1252599548343744)



### Python

- [ ] [Python教程 - 廖雪峰的官方网站](https://www.liaoxuefeng.com/wiki/1016959663602400)







## 数据

[SQL教程 - 廖雪峰的官方网站](https://www.liaoxuefeng.com/wiki/1177760294764384)

## 测试

### SQL 



### MySQL 



### Redis



## 运维

linux => docker => nginx

### Shell



### Linux



### Docker



### Nginx

[bregman-arie/devops-exercises](https://github.com/bregman-arie/devops-exercises)

[AmShadows/Linux: Linux运维](https://github.com/AmShadows/Linux)

[jaywcjlove/linux-command: Linux命令大全搜索工具，内容包含Linux命令手册、详解、学习、搜集。https://git.io/linux](https://github.com/jaywcjlove/linux-command)

[dunwu/linux-tutorial: Linux教程，主要内容：Linux 命令、Linux 系统运维、软件运维、精选常用Shell脚本](https://github.com/dunwu/linux-tutorial)
[mengning/linuxstart: Linux从零入门实战 —— 边学边练：最有效的Linux实践之路](https://github.com/mengning/linuxstart)
[xirong/my-linux: 个人学习linux的一些资料收集](https://github.com/xirong/my-linux)
[wenquan0hf/linux: Linux 入门教程](https://github.com/wenquan0hf/linux)

## 实战

### PC桌面开发



### APP开发



### 响应式网页



### 微信小程序



### 微信公众号





## 开发

- 2020前端开发学习路线[omi/rm.md at master · Tencent/omi](https://github.com/Tencent/omi/blob/master/assets/rm.md)

### 设计图





### 流程图





## 面试

### 简历包装

- [ ] 推荐使用：[CodingDocs/typora-markdown-resume: typora+markdown+css自定义简历模板](https://github.com/CodingDocs/typora-markdown-resume)

- [x] [dyweb/awesome-resume-for-chinese: 适合中文的简历模板收集（LaTeX，HTML/JS and so on）由 @hoochanlon 维护](https://github.com/dyweb/awesome-resume-for-chinese)
  - [ ] [mdnice/markdown-resume: 支持 Markdown 和富文本的在线简历排版工具](https://github.com/mdnice/markdown-resume)
- [ ] [10 个 GitHub 上最火的程序员简历项目，2021 金三银四必备的加薪宝剑！ - SegmentFault 思否](https://segmentfault.com/a/1190000038571000)

- [ ] README 美化[anuraghazra/github-readme-stats: Dynamically generated stats for your github readmes](https://github.com/anuraghazra/github-readme-stats)
  - [ ] [resumejob/awesome-resume: Resume，Resume Templates，程序员简历例句，简历模版，](https://github.com/resumejob/awesome-resume#%E4%B8%AD%E6%96%87)

### 面试题

英文

- [ ] [jwasham/coding-interview-university: A complete computer science study plan to become a software engineer.](https://github.com/jwasham/coding-interview-university)
- [ ] [yangshun/tech-interview-handbook: 💯 Curated interview preparation materials for busy engineers](https://github.com/yangshun/tech-interview-handbook)
- [yangshun/front-end-interview-handbook: ⚡️ Front End interview preparation materials for busy engineers](https://github.com/yangshun/front-end-interview-handbook)
- [poteto/hiring-without-whiteboards: ⭐️ Companies that don't have a broken hiring process](https://github.com/poteto/hiring-without-whiteboards)
- [kdn251/interviews: Everything you need to know to get the job.](https://github.com/kdn251/interviews)
- [Introduction · 笔试面试知识整理](https://hit-alibaba.github.io/interview/index.html)

中文

- [huihut/interview: 📚 C/C++ 技术面试基础知识总结，包括语言、程序库、](https://github.com/huihut/interview)
- [yifeikong/reverse-interview-zh: 技术面试最后反问面试官的话](https://github.com/yifeikong/reverse-interview-zh)
- [linw7/Skill-Tree: 🐼 准备秋招，欢迎来树上取果实](https://github.com/linw7/Skill-Tree)
- [shfshanyue/Daily-Question: 互联网大厂内推及大厂面经整理，并且每天一道面试题推送。每天五分钟，半年大厂中](https://github.com/shfshanyue/Daily-Question)
- [ArthurZheng/Algorithm_Interview_Notes-Chinese: 2018/2019/校招/春招/秋招/算法/机器学习(Machine Learning)/深度学习(Deep Learning)/自然语言处理(NLP)/C/C++/Python/面试笔记](https://github.com/ArthurZheng/Algorithm_Interview_Notes-Chinese)
- [Advanced-Frontend/Daily-Interview-Question: 我是依扬（木易杨），公众号「高级前端进阶」作者，每天搞定一道前端大厂面试题，祝大家天天进步，一年后会看到不一样的自己。](https://github.com/Advanced-Frontend/Daily-Interview-Question)

### Leetcode

- [azl397985856/leetcode: LeetCode Solutions: A Record of My Problem Solving Journey.( leetcode题解，记录自己的leetcode解题之路。)](https://github.com/azl397985856/leetcode)
- [algorithm-visualizer/algorithm-visualizer: Interactive Online Platform that Visualizes Algorithms from Code](https://github.com/algorithm-visualizer/algorithm-visualizer)
- [haizlin/fe-interview: 前端面试每日 3+1](https://github.com/haizlin/fe-interview)
  [GitbookIO/gitbook: 📝 Modern documentation format and toolchain using Git and Markdown](https://github.com/GitbookIO/gitbook)
- [MisterBooo/LeetCodeAnimation: Demonstrate all the questions on LeetCode in the form of animation.（用动画的形式呈现解LeetCode题目的思路）](https://github.com/MisterBooo/LeetCodeAnimation)
- [geekxh/hello-algorithm](https://github.com/geekxh/hello-algorithm)
- [wangzheng0822/algo: 数据结构和算法必知必会的50个代码实现](https://github.com/wangzheng0822/algo)

- [haoel/leetcode: LeetCode Problems' Solutions](https://github.com/haoel/leetcode)

- [afatcoder/LeetcodeTop: 汇总各大互联网公司容易考察的高频leetcode题🔥](https://github.com/afatcoder/LeetcodeTop)

- [overnote/over-algorithm: 数据结构与算法：[进度40%]以C语言严谨描述数据结构与算法，同时以Go、JavaScript、C++等语言以简洁方式实现](https://github.com/overnote/over-algorithm)
- [iostalks/Algorithms: 数据结构和算法专项训练营。✍️✍️✍️](https://github.com/iostalks/Algorithms)

- [LeechanX/Data-Structures-and-Algorithms-in-C: 所有基础数据结构和算法的纯C语言实现，如各自排序、链表、栈、队列、各种树以及应用、图算法、字符串匹配算法、回溯、并查集等，献丑了](https://github.com/LeechanX/Data-Structures-and-Algorithms-in-C)
- [iwts/data-structure: 数据结构与算法，用C与Java实现。](https://github.com/iwts/data-structure)

## 数学

### 高等数学



### 微积分



### 概率论与数理统计



### 线性代数



## 英语







## 项目

API

- [10个打开了我新世界大门的 WebAPI](https://juejin.cn/post/6844904193258602504#heading-2)
- [【开发利器】中国国内可用API合集 | 涵月天-microzz.com | IT技术分享](https://microzz.com/2017/02/03/API/)
- [今日诗词开放接口 - 今日诗词 - 诗词实时智能推荐](https://www.jinrishici.com/doc/#json-fast)
- [核心接口 | 一言开发者中心](https://developer.hitokoto.cn/core/#%E8%BF%94%E5%9B%9E%E7%BA%A6%E6%9D%9F)
- [花海API-免费数据接口调用平台](https://pis.junsangs.com/)

- [public-apis/public-apis: A collective list of free APIs](https://github.com/public-apis/public-apis)
- [nswbmw/N-blog: 《一起学 Node.js》](https://github.com/nswbmw/N-blog)
  [yygmind/blog: 我是木易杨，公众号「高级前端进阶」作者，跟着我每周重点攻克一个前端面试重难点。接下来让我带你走进高级前端的世界，在进阶的路上，共勉！](https://github.com/yygmind/blog)



- [pretty-git/Fashion-Style-normal: 一款关于穿搭的微信小程序，有海报轮播图、分类swiper、搜索功能、人工智能api图像识别、评论、收藏模块、分享海报、上传图片，天气模块](https://github.com/pretty-git/Fashion-Style-normal)



项目

- [awesome-selfhosted](https://github.com/awesome-selfhosted/awesome-selfhosted)：该仓库收集了 1000+ 可本地搭建的网络服务和 Web 应用程序
- [Web-Dev-For-Beginners:](https://github.com/microsoft/Web-Dev-For-Beginners)：适合初学者的 Web 开发教程
- [Github实战项目 - GitHub中文社区](https://www.githubs.cn/real-world)



- 时钟 全屏
- 

## 扩展

网络安全

编译原理

计算机科学

设计模式

计算机科学与导论

## 游戏

- [Flexbox Froggy - A game for learning CSS flexbox](https://flexboxfroggy.com/)
- [挑战CodeCombat关卡 - 学习Python，JavaScript和C++ | CodeCombat](https://codecombat.com/play)
- [CheckiO - coding games and programming challenges for beginner and advanced](https://checkio.org/)
- [Grid Garden - 一个用来学CSS grid的游戏](http://cssgridgarden.com/#zh-cn)
- [CSS Diner - Where we feast on CSS Selectors!](https://flukeout.github.io/)
- [Service Workies](https://serviceworkies.com/)
- [CodeGym is an online programming course to learn Java - Sign in](https://codegym.cc/login)
- [Learn Git Branching](https://learngitbranching.js.org/?locale=zh_CN)
- [Botzone](https://www.botzone.org.cn/)
- [fightcode.io](http://fightcode.io/)
- [Codewars - Achieve mastery through coding practice and developer mentorship](https://www.codewars.com/)
- [扣哒世界|CodeCombat|AI世青赛|少儿编程等级认证](https://codecombat.cn/)
- [CheckiO - coding games and programming challenges for beginner and advanced](https://checkio.org/)
- [Learn VIM while playing a game - VIM Adventures](https://vim-adventures.com/)
- [cyber-dojo](https://www.cyber-dojo.org/creator/home)
- [Coding for Kids | Game-Based Programming | CodeMonkey](https://www.codemonkey.com/)
- [Elevator Saga - the elevator programming game](http://play.elevatorsaga.com/)
- [Codewars - Achieve mastery through coding practice and developer mentorship](https://www.codewars.com/)
- [Ruby Quiz](http://rubyquiz.com/)
- [hacker.org - The Hacker Community Online](http://www.hacker.org/)
- [Code Hunt - Microsoft Research](https://www.microsoft.com/en-us/research/project/code-hunt/)

## 网站

| 网址                                                         | 描述                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| [unbug.github.io/codelf](https://link.juejin.cn?target=https%3A%2F%2Funbug.github.io%2Fcodelf%2F) | 变量命名智能推荐（支持中文）                                 |
| [www.docschina.org](https://link.juejin.cn?target=https%3A%2F%2Fwww.docschina.org) | Web 前端开发人员提供优质中文文档                             |
| [regexr.com](https://link.juejin.cn?target=https%3A%2F%2Fregexr.com%2F) | 正则表达式验证匹配                                           |
| [any-rule](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Fany86%2Fany-rule) | 正则表达式库，非常全，使用起来很方便                         |
| [www.toptal.com/developers](https://link.juejin.cn?target=https%3A%2F%2Fwww.toptal.com%2Fdevelopers%2Fcss%2Fsprite-generator) | 多张图片合成雪碧图，并生成对应 css                           |
| [tool.lu](https://link.juejin.cn?target=https%3A%2F%2Ftool.lu%2F) | 众多工具集合，包括时间戳转换，进制转换等                     |
| [www.bootcdn.cn](https://link.juejin.cn?target=https%3A%2F%2Fwww.bootcdn.cn%2F) 或 [cdn.baomitu.com](https://link.juejin.cn?target=https%3A%2F%2Fcdn.baomitu.com%2F) | 国内的CDN库，速度快                                          |
| [www.jsdelivr.com](https://link.juejin.cn?target=https%3A%2F%2Fwww.jsdelivr.com) | 国外的 cdn 库，支持 github，npm，WordPress                   |
| [www.gitignore.io](https://link.juejin.cn?target=https%3A%2F%2Fwww.gitignore.io%2F) | 根据选择会去生成 .gitignore 文件                             |
| [codesandbox.io](https://link.juejin.cn?target=https%3A%2F%2Fcodesandbox.io%2F) | 在线编辑代码                                                 |
| [www.typora.io](https://link.juejin.cn?target=https%3A%2F%2Fwww.typora.io) | 实用的 Markdown 写作工具，所见即所得                         |
| [mdnice.com](https://link.juejin.cn?target=https%3A%2F%2Fmdnice.com) | 使 markdown 语法更加美观，如果你有写博客，那这个非常合适你(强烈推荐) |



### 检查测试

检测Web前端兼容性或者是网站性能测试

| 网址                                                         | 说明                                                   |
| ------------------------------------------------------------ | ------------------------------------------------------ |
| [Can I use ](https://link.juejin.cn?target=https%3A%2F%2Fcaniuse.com%2F) | 正如网站一样，我这个属性能在哪些浏览器兼容，一查便知。 |
| [Browser Sandbox](https://link.juejin.cn?target=https%3A%2F%2Fturbo.net%2Fbrowsers) | 各大平台浏览器兼容性在线测试                           |
| [GTmetrix](https://link.juejin.cn?target=https%3A%2F%2Fgtmetrix.com%2F) | 网站在线性能测试分析、优化建议                         |
| [HttpWatch](https://link.juejin.cn?target=https%3A%2F%2Fwww.httpwatch.com%2F) | 一款强大的网页HTTP数据分析软件                         |
| [PageSpeed](https://link.juejin.cn?target=http%3A%2F%2Fdevelopers.google.com%2Fspeed%2Fpagespeed%2Finsights%2F) | Google在线性能测试网站、优化建议                       |
| [validator](https://link.juejin.cn?target=http%3A%2F%2Fvalidator.w3.org%2F) | W3C的HTML有效性验证                                    |
| [css-validator](https://link.juejin.cn?target=http%3A%2F%2Fjigsaw.w3.org%2Fcss-validator%2F) | W3C的CSS有效性验证                                     |

设计规范
iPhone各设备屏幕尺寸
移动屏幕尺寸设计规范
微信官方小程序UI样式库
各大尺寸设备响应式屏幕效果显示





[CodeSandbox: Online Code Editor and IDE for Rapid Web Development](https://codesandbox.io/)

## 博客

文档构建工具

vuepress

[快速上手 | VuePress](https://vuepress.vuejs.org/zh/guide/getting-started.html)

[借助 VuePress 和 GitBook ，10 分钟即可免费部署你的静态博客网站 - 云+社区 - 腾讯云](https://cloud.tencent.com/developer/article/1556820)

```bash
# 安装
yarn global add vuepress # 或者：npm install -g vuepress
# 创建项目目录
mkdir vuepress-starter && cd vuepress-starter
# 新建一个 markdown 文件
echo '# Hello VuePress!' > README.md
{
  "scripts": {
    "docs:dev": "vuepress dev docs",
    "docs:build": "vuepress build docs"
  }
}
yarn docs:dev # npm run docs:dev
# 开始写作
vuepress dev .
# 构建静态文件
vuepress build .
```

vuepress模板

```
git clone https://github.com/openHacking/vuepress-template.git
cd vuepress-template
yarn # 或者npm i
npm run docs:dev
yarn 
npm run docs:build
```

vuepress主题



github action





gitbook

[介绍 | GitBook文档（中文版）](https://chrisniael.gitbooks.io/gitbook-documentation/content/)

[gitbook 入门教程之使用 gitbook-cli 开发电子书 - 掘金](https://juejin.cn/post/6844903812369481741)

[GitBook - 快速打造可留言的博客 - 掘金](https://juejin.cn/post/6844903848914452488)

[dunwu/gitbook-templates: gitbook 学习笔记 & 自用模板](https://github.com/dunwu/gitbook-templates)

```bash
gitbook 生成电子书主要有三种方式:

gitbook-cli 命令行操作,简洁高效,适合从事软件开发的相关人员.
gitbook-editor 编辑器操作,可视化编辑,适合无编程经验的文学创作者.
gitbook.com 官网操作,在线编辑实时发布,适合无本地环境且科学上网的体验者.

安装 GitBook：npm i gitbook-cli -g
初始化 GitBook 项目：gitbook init
安装 GitBook 依赖：gitbook install
开启 GitBook 服务：gitbook serve
打包 GitBook 项目：gitbook build
GitBook 命令行查看：gitbook -help
GitBook 版本查看：gitbook -V
```

docsify





hexo





wordpress
